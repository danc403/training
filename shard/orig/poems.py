...
def run_poem_sharding(is_tune_mode):
    ...
    file_metadata = {}
    print(f"\nPRE-SCANNING POEM DATASETS FOR {output_dir.upper()}...")
    ...
    while active_files:
        ...
        if POEM_MIN_TOKENS <= len(tks_raw) <= POEM_MAX_TOKENS:
            # We respect the weight by packing multiple variants if needed
            for _ in range(meta["weight"]):
                if is_tune_mode:
                    p_str = get_poem_prompt(title)
                    t_q = tokenizer.encode(p_str, add_special_tokens=False).ids
                    
                    # Structural injection: <|user|>Prompt\n\n<|assistant|>PoemText
                    full_tks = [user_id] + t_q + [newline_id, newline_id, asst_id] + tks_raw
                    
                    # Mask everything through the assistant marker
                    full_msk = [0] * (len(t_q) + 5) + [1] * len(tks_raw)
                    pack_sequence(full_tks, full_msk)
                else:
                    # Base mode: Raw text with random noise
    ...
    f_d.close(); f_m.close()
    print(f"COMPLETED {output_dir.upper()} POEM SHARDING.")
