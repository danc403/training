...
if [ "$action" == "up" ]; then
    ...
    rsync -avz -e "ssh -p $vast_port -o ServerAliveInterval=60 -o ServerAliveCountMax=10" ./train.sh setup.sh tune.sh eval.py eval2.py requirements.txt trainer base instruct tokenizer root@$vast_host:/root/vast/
