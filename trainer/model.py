...
# Aliasing for code compatibility, but we will use standard layers
import torch.nn as te

# Compatibility Check: Native RMSNorm was added in Torch 2.1.
# If your Debian rig is older, we define it here so the script doesn't die.
...
class NymphBlock(nn.Module):
    ...
    Reverted to Native Torch for maximum compatibility.
    """
    def __init__(self, config, rope_module, layer_idx=0):
        ...
        # Attention Projections: Removed params_dtype
    ...
    def forward(self, x, past_key_value=None, mask=None):
        ...
        # Native SDPA (Handled automatically by Torch)
...
class NymphModel(nn.Module):
    ...
    def forward(self, input_ids, past_key_values=None, mask=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.training and past_kv is None:
