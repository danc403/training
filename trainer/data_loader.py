...
class MaskedBinaryShardDataset(Dataset):
    def __init__(self, data_dir, context_length, eos_id=0, device="cuda"):
        ...
        # Pre-load all data into memory (GPU or CPU based on device)
        self.all_data = []
        self.all_masks = []
        ...
        print(f"--- Pre-loading {len(data_paths)} shards into {self.device.upper()} ---")
        for dp, mp in zip(data_paths, mask_paths):
            ...
            # Dynamically calculate blocks based on context_length variable
            blocks_in_shard = num_tokens // self.context_length
        ...
        print(f"--- Data load complete. Total blocks: {self.num_samples} ---")
    ...
    def __getitem__(self, idx):
        ...
        start_idx = local_block_idx * self.context_length
        end_idx = start_idx + self.context_length
        
        # Slicing from current device memory
        ...
        if self.extra_mask_chance > 0:
            noise = torch.rand(self.context_length, device=self.device)
...
def get_dataloader(data_dir, context_length, batch_size, eos_id=0, num_workers=0, device="cuda", force_cpu_loader=False):
    """
    Returns a DataLoader with dynamic device selection and memory pinning.
    force_cpu_loader: If True, forces data residency in RAM even if training device is GPU.
    """
    # Normalize device: if device is any GPU variant, treat it as a GPU-capable device
    is_gpu = device in ["cuda", "rocm", "hip", "hipc"]
    loader_device = "cpu" if (force_cpu_loader or not is_gpu) else "cuda"
    
    dataset = MaskedBinaryShardDataset(data_dir, context_length, eos_id=eos_id, device=loader_device)
